...
const makeFeatureApi = (root, identity, ui) => {
	...
	const api = {
		...
		read: (rel) =>
			fs.existsSync(join(rel))
				? fs.readFileSync(join(rel), 'utf8')
				: null,
		...
		remove(rel) {
			...
			if (fs.statSync(abs).isDirectory()) {
				throw new Error(
					`remove(): refusing to delete directory "${rel}"; remove files individually so rollback can restore them.`
				);
			}
		...
		writeFlag(key, on) {
			...
			const oldRaw = fs.existsSync(abs)
	...
	if (apply.scripts) {
		...
		Object.entries(apply.scripts).forEach(([name, cmd]) => {
			if (undefined === scripts[name]) {
				...
				ui.warn(
					`Script "${name}" already exists; keeping the current value.`
				);
			}
		});
	}
...
const unmergePackage = (pkg, apply, survivors = []) => {
	...
	survivors.forEach((feature) => {
		...
		Object.keys(a.dependencies || {}).forEach((name) =>
			ownedDeps.add(name)
		);
		Object.keys(a.devDependencies || {}).forEach((name) =>
			ownedDeps.add(name)
		);
...
const detectFeature = (feature, api) => {
	...
	return (
		files.every((file) => api.exists(file.to)) &&
		deps.every((dep) => api.hasDep(dep))
	);
};
...
const enableFeature = (feature, api, featuresDir) => {
	runJournaled(api, () => {
		...
		(apply.files || []).forEach((file) => {
			const srcAbs = resolveWithin(
				path.join(api.root, featuresDir),
				file.from
			);
 ...
 * @return {Promise<{changed: boolean, failed: string[], finalMap: Object}>} Whether anything changed, the keys that failed, and the post-apply detected map.
 */
const toggleFeatures = async (config, root, opts) => {
	...
	if (!rows) {
		const r = reconcile(
			config,
			(api.identity && api.identity.features) || {},
			api
		);
		rows = r.rows;
		unknown = r.unknown;
	...
	const finalize = (changed, failed) => {
		...
		if (failed && failed.length) {
			process.exitCode = 1;
			ui.warn(
				`Some features failed: ${failed.join(', ')}. Re-run init to retry.`
			);
		}
	...
	rows.filter((r) => r.drift).forEach((r) =>
		ui.warn(
			`${r.key}: recorded ${r.intent ? 'enabled' : 'disabled'} but disk says ${r.on ? 'enabled' : 'disabled'}; using disk state.`
		)
	);
	(unknown || []).forEach((key) =>
		ui.warn(`${key}: retired feature still recorded; leaving untouched.`)
	);
	...
	if (!wantOn) {
		...
		for (;;) {
			ui.table(
				rows.map((r) => [
					r.label,
					`${wantOn.has(r.key) ? 'enabled' : 'disabled'}${r.drift ? '  (drift)' : ''}`,
				]),
				{ title: 'Optional features' }
			);
			const choiceFor = (r) =>
				`${wantOn.has(r.key) ? '[x]' : '[ ]'} ${r.label}`;
