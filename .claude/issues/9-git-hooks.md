...
- `src/cli/index.js` — edited (`main()` is now declared `async` for a uniform `Promise<number>` contract; matches `v1.0.0/task/scaffold-registry` byte-for-byte)
...
- `tests/cli/index.test.js` — edited (all `cli.main()` tests converted to async/await; cover `install-hooks` routing + registry presence)
...
> jest
 ...
 PASS  tests/hooks/install.test.js
  installHooks
    ✓ installs both hooks with the executable bit set (22 ms)
    ...
    ✓ resolves the hooks dir when .git is a file (separate git dir) (17 ms)
    ✓ onConflict callback receives the conflict info per template (21 ms)
    ✓ onConflict returning true overwrites the existing hook (21 ms)
    ✓ --force bypasses the onConflict callback entirely (21 ms)
    ✓ --dry-run reports skips for conflicts without consulting onConflict (20 ms)
    ✓ onBeforeWrite / onAfterWrite fire around each successful install (16 ms)
  injectVersionHeader
  ...
  install-hooks runCli
    ...
    ✓ non-TTY conflict skips without prompting (CI-safe) (20 ms)
 ...
 PASS  tests/hooks/templates.test.js
  commit-msg template
    ...
    ✓ accepts "ci(detect-changes): cover edge case" (1 ms)
  ...
  pre-commit template
    ✓ no-ops when package.json is absent (3 ms)
    ✓ no-ops when package.json has no lint:staged script (5 ms)
  shellcheck (optional)
    ✓ commit-msg passes shellcheck -s sh (44 ms)
 ...
 PASS  tests/cli/index.test.js
  ...
  cli loadCommands()
    ...
    ✓ returns entries in deterministic (sorted) order (7 ms)
 ...
 PASS  tests/ui/selects.test.js
  ...
  flat select validation
    ✓ should throw when checkbox choices is missing (3 ms)
